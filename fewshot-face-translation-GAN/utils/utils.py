...
def get_68_landmarks_edge_image(img, face, lms, eyes_binary_mask=False, apply_dilate_or_erode=True):
    ...
    for i in range(27,30):
        ...
        result = cv2.line(result.copy(), (y0, x0), (y1, x1), (0,255,0), stroke_size*2)
    return result
...
def get_src_inputs(fn, fd, fp, idet):
    ...
    aligned_im,M = align_image(im, x0, y0, x1, y1, landmarks)
    ...
    return aligned_face, parsing_map_with_iris, aligned_im, (x0, y0, x1, y1), landmarks, M
...
def get_tar_inputs(fns, fd, fv):
    ...
    for fn in fns:
        ...
        im = auto_resize(im)
        (x0, y0, x1, y1), landmarks = detect_face(im, fd)
        aligned_im,_ = align_image(im, x0, y0, x1, y1, landmarks)    
...
def post_process_result(fn, fd, result, aligned_im, src, x0, y0, x1, y1, landmarks, M):
    ...
    output_im = cv2.warpAffine(output_im,OKMM,(cols,rows),flags = cv2.INTER_NEAREST)
    
    
    
    dst = np.where(output_im==np.array([0,0,0]),im,output_im)
    ...
    # al=align_image(output_im,fx0, fy0, fx1, fy1,landmarks)
   
    # al = np.where(output_im==np.array([0,0,0]),output_im,im)
